...
`notifyOrderStatus` is the one-account-to-one-account case: when an admin changes an order's `status` to `confirmed`, `fulfilled` or `cancelled`, only that order's `userId` is notified (subject to their own alert opt-in, same as the others). It fires once per status value reached, using the Firestore change event ID as the delivery-claim key, so re-triggers on unrelated field writes or repeat deliveries never double-send.
